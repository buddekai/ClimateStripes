...
plotWarmingStripes <- function(df.temp, startyear.mean, endyear.mean,
  ...
  mean.from.start.to.endyear <- mean(
    df.temp.annual$mean[
      df.temp.annual$year>= startyear.mean &
        df.temp.annual$year<= endyear.mean])

  df.temp.annual$deviations <-
    df.temp.annual$mean - mean.from.start.to.endyear
  ...
  print(paste("Year with highest negative deviation from mean: ",
              ...
              round(df.temp.annual$mean[
                which(df.temp.annual$deviations ==
                        min(df.temp.annual$deviations))],
                digits = 1),
              "\u00B0C.",
              sep=""))
  ...
  deviation.range <- max(df.temp.annual$deviations) -
    min(df.temp.annual$deviations)
  deviation.range <- 2*max(abs(min(df.temp.annual$deviations)),
                           abs(max(df.temp.annual$deviations)))

  temp.lables <- replicate(11, NaN)
  for(i in 1:11){
    for(j in 1:length(df.temp.annual$year)){
      if(df.temp.annual$deviations[j] >= (-0.5*deviation.range +
  ...
  if(style == "discrete"){
    plot.warmingStripes <-
      ...
      scale_fill_manual(values = rev(col_strip)[
        sort(as.integer(levels(unique(df.temp.annual$deviationscat))))],
  ...
  print(paste("Plot save in ", getwd(), ".", sep=""))
